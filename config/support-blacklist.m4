...
AC_DEFUN([ROSE_SUPPORT_BLACKLIST],[
    ...
    while true; do
        ...
        dnl Boost-1.62 and 1.63 serialization cause link errors when compiled by Intel compilers. Binary analysis
        dnl is the only thing that uses serialization at this time.
        if test "$rose_boost_version" -ge 106200 -a "$rose_boost_version" -le 1063000 -a \
